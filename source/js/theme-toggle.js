(function() {
  const STORAGE_KEY = 'zen-theme-mode';
  const THEME_LIGHT = 'light';
  const THEME_DARK = 'dark';
  const ANIMATION_CLASS = 'theme-animating';
  const ANIMATION_DURATION_MS = 320;

  const SUN_SVG = '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line></svg>';
  const MOON_SVG = '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>';

  let animationTimer = null;

  function supportsReducedMotion() {
    return window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  }

  function getStoredTheme() {
    try {
      return localStorage.getItem(STORAGE_KEY);
    } catch (e) {
      return null;
    }
  }

  function setStoredTheme(theme) {
    try {
      localStorage.setItem(STORAGE_KEY, theme);
    } catch (e) {
      // Ignore storage write failures.
    }
  }

  function getSystemTheme() {
    return window.matchMedia('(prefers-color-scheme: dark)').matches ? THEME_DARK : THEME_LIGHT;
  }

  function getEffectiveTheme() {
    return getStoredTheme() || getSystemTheme();
  }

  function getCurrentTheme() {
    return document.documentElement.getAttribute('data-theme') || THEME_LIGHT;
  }

  function startThemeAnimation() {
    if (supportsReducedMotion()) {
      return;
    }

    const root = document.documentElement;
    root.classList.add(ANIMATION_CLASS);

    if (animationTimer) {
      window.clearTimeout(animationTimer);
    }

    animationTimer = window.setTimeout(function() {
      root.classList.remove(ANIMATION_CLASS);
    }, ANIMATION_DURATION_MS);
  }

  function updateButtonIcon(theme) {
    const btn = document.getElementById('theme-toggle');
    if (!btn) {
      return;
    }

    btn.setAttribute('aria-label', theme === THEME_DARK ? '切换到浅色模式' : '切换到深色模式');
    btn.setAttribute('data-theme-toggle-target', theme === THEME_DARK ? THEME_LIGHT : THEME_DARK);
    btn.innerHTML = theme === THEME_DARK ? SUN_SVG : MOON_SVG;
  }

  function emitThemeChange(theme, source) {
    document.dispatchEvent(
      new CustomEvent('theme-change', {
        detail: {
          theme: theme,
          source: source || 'unknown'
        }
      })
    );
  }

  function applyTheme(theme, options) {
    const opts = options || {};
    const previous = getCurrentTheme();

    document.documentElement.setAttribute('data-theme', theme);
    document.documentElement.classList.toggle('dark', theme === THEME_DARK);

    if (opts.withAnimation && previous !== theme) {
      startThemeAnimation();
    }

    updateButtonIcon(theme);

    if (opts.emitEvent !== false) {
      emitThemeChange(theme, opts.source || 'apply');
    }
  }

  function toggleTheme() {
    const next = getCurrentTheme() === THEME_DARK ? THEME_LIGHT : THEME_DARK;
    setStoredTheme(next);
    applyTheme(next, { withAnimation: true, source: 'manual-toggle' });
  }

  function bindSystemThemeSync() {
    const media = window.matchMedia('(prefers-color-scheme: dark)');

    function syncWithSystem() {
      if (getStoredTheme()) {
        return;
      }
      applyTheme(getSystemTheme(), { withAnimation: true, source: 'system' });
    }

    if (typeof media.addEventListener === 'function') {
      media.addEventListener('change', syncWithSystem);
    } else if (typeof media.addListener === 'function') {
      media.addListener(syncWithSystem);
    }
  }

  function init() {
    applyTheme(getEffectiveTheme(), { emitEvent: false, source: 'init' });

    const btn = document.getElementById('theme-toggle');
    if (btn) {
      btn.addEventListener('click', toggleTheme);
    }

    bindSystemThemeSync();
  }

  document.addEventListener('DOMContentLoaded', init);
})();
